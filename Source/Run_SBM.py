...
h = 2
perturb = 0.3
#lambdas = [0, 0.001, 0.01, 0.1, 0.22, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.5, 3, 4, 5, 6, 7, 8, 9, 10]
lambdas = [0, 0.001, 0.1, 0.5, 1, 2, 3, 5, 8, 10, 20, 50, 100, 1000]

df_FNMTF = pd.DataFrame(columns=['network','attr','method','nodes (n)','clusters (k)','lam', 'groups (h)','accurcay','average balance'])
...
for nodes in n:
    ...
    for num_c in k:
        for lam in lambdas:
            num_repeats = 10
            Acc_iter_fnmf, B_iter_fnmf = [], []

            adj_mat, fair_mat, clusters, groups = gen_kleindessner(nodes, num_c, h, a, b, c, d)
            n_swap = np.round(perturb * nodes/h).astype(int)
            ind = np.where(groups == 1)[0]
            groups[np.random.choice(ind, size=n_swap, replace=False)] = 0
            ...
            results = dict()
            results = {'network': "SBM", 'attr': "distributed", 'method': "ifair_nmtf", 'nodes (n)': nodes,
                       'clusters (k)': num_c, 'lam': lam, 'groups (h)': h, 'accurcay': Acc_FNMTF,
                       'average balance': B_FNMTF}
...
#display(df_FNMF)
print(df_FNMTF)
df_FNMTF.to_csv('Fair_NMTF_SBM_perturb_k_lam_gridsearch' + '.csv', index=False)

# -------------------------------- conditioned SBM perturbation comparisons ----------------------------------------------

df_comp = pd.DataFrame(columns=['network','attr','method','nodes (n)','clusters (k)', 'groups (h)', 'a = 0.7 log(n)/n**2/3$', 'accurcay','average balance'])
...
#display(df_comp)
print(df_comp)
df_comp.to_csv('SBM_perturb_k_n_gridsearch' + '.csv', index=False)