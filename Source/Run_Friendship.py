...
for num_c in k:
    for lam in lambdas:
        ...
        results = dict()
        results = {'network': "Friendship", 'attr': "Gender", 'method': "FairNMF", 'lam': lam, 'nodes (n)': n,
...
df1.to_csv('Friendship_k_lam_gridsearch_FNMTF.csv', index=False)
print (df1)
...
for num_c in k:
    ...
    # row of results to be appended to df
    col1 = ["Friendship" for i in range(3)]
...
print(df)
df.to_csv('Friendship.csv', index=False)
"""
