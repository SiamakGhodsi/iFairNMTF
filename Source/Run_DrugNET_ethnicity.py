...
all_in_one = np.ones(F_ethn.shape[0])
uniqe_vals, count = np.unique(F_ethn, return_counts=True)
DrugNet_balance_ethnicity = min(count)/max(count)

print(len(F_ethn), len(A))
...
for num_c in k:
    for lam in lambdas:
        ...
        results = dict()
        results = {'network': "DrugNet", 'attr': "Ethnicity", 'method': "FairNMF", 'lam': lam, 'nodes (n)': n,
...
df1.to_csv('DrugNET_ethnicity_k_lam_gridsearch_FNTMF.csv', index=False)
...
for num_c in k:
    ...
    # row of results to be appended to df
    col1 = ["DrugNet" for i in range(3)]
    col2 = ["Ethnicity" for i in range(3)]
