...
standard_groups = reflow_clusters(F_lastFM.ravel())
...
for num_c in k:
    for lam in lambdas:
        ...
        results = dict()
        results = {'network': "LastFM", 'attr': "Gender", 'method': "FairNMF", 'lam': lam, 'nodes (n)': n,
        ...
        print(temp)
    if num_c == 10:
        df1.to_csv('LastFM_k_lam_gridsearchk_10.csv', index=False)

df1.to_csv('LastFM_k_lam_gridsearch.csv', index=False)
...
for num_c in k:
    ...
    # row of results to be appended to df
    col1 = ["LastFM" for i in range(3)]
    col2 = ["Country of Origin" for i in range(3)]
...
print(df)
df.to_csv('LastFM.csv', index=False)
"""
